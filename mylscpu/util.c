...
void add_proc_list(char path[PATH_LEN], bool isMYPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
	...
	long double cpu = 0;
	bool update = false;
	if(!isMYPS && cpuTimeTable != NULL){			//mytop일 경우
	...
	if(isMYPS){		//MYPS인 경우에만 세부 state 추가
	...
	if(!isMYPS || (!aOption && !uOption && !xOption))	//mytop이거나 myps에서 옵션이 없을 경우
...
void search_proc(bool isMYPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
	...
	while((dentry = readdir(dirp)) != NULL){	// /proc 디렉터리 내 하위 파일들 탐색 시작
		...
		if(!isPid)				//PID 디렉터리가 아닌 경우 skip
			continue;
		if(isMYPS && !aOption)			//aOption이 없을 경우 자기 자신의 process만 보여줌
			if(statbuf.st_uid != myUid)		//uid가 자기 자신과 다를 경우 skip
				continue;
		if(isMYPS && !xOption){			//xOption이 없을 경우 nonTerminal process는 생략함
			char tty[TTY_LEN];
			memset(tty, '\0', TTY_LEN);
		...
		if(isMYPS && !aOption && !uOption && !xOption){	//모든 Option 없을 경우
			char tty[TTY_LEN];
			memset(tty, '\0', TTY_LEN);
			getTTY(path, tty);		//TTY 획득
		...
		add_proc_list(path, isMYPS, aOption, uOption, xOption, cpuTimeTable);	//PID 디렉터리인 경우 procList에 추가
	}
	closedir(dirp);
	return;
}
